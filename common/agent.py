...
class Agent:
    def __init__(self, env_args: Config, model_args: Config, buffer_args: Config, train_args: Config):
        ...
        if self.model_args['load'] is None:
            ...
            if '/' in self.model_args['load'] or '\\' in self.model_args['load']: 
                self.train_args['load_model_path'] = self.model_args['load']
            elif '-' in self.model_args['load']:
                self.train_args['load_model_path'] = os.path.join(self.train_args['base_dir'], self.model_args['load']) 
            ...
            if self.model_args['algo'] in ['drqn', 'drdqn']:
                ...
                if _use_priority and _n_step:
                    ...
                    if self.train_args['sem']:
                        ...
                        self.use_sem = True
                        self.buffer_args['SEM']['gamma'] = algorithm_config['gamma']
                        algorithm_config['gamma'] = pow(algorithm_config['gamma'], self.buffer_args['SEM']['n'])
                    else:
    ...
    def gym_train(self):
        ...
        for episode in range(begin_episode, max_episode):
                ...
                if policy_mode == 'off-policy':
                    ...
                    if off_policy_step_eval and total_step % eval_interval == 0:
                        ...
                        if total_step % 1000 == 0:
                            self.pwi(f'total_step: {total_step}')
                total_step += 1
            ...
            if self.use_sem:
                self.model.data.mark(r, sma.mean, sma.fluct)
            self.pwi('-' * 40)
    ...
    def unity_train(self):
        ...
        for episode in range(begin_episode, max_episode):
            ...
            for i in range(self.env.brain_num):
                ...
                if self.use_sem:
                    self.models[i].data.mark(rewards[i], sma[i].mean, sma[i].fluct)
            self.pwi('-' * 40)
